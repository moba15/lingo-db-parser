...
# GoogleSQL 
## May Add?
- EXTEND `FROM lineitem |> EXTEND l_quantity * l_extendedprice AS cost |> EXTEND cost * l_discount AS discount |> WHERE discount > 1000 |> AGGREGATE SUM(cost), SUM(discount)`
- FROM standalone is allowed
- Multiple AGGREGATES (maybe add node AGGREGATE which has a list of AGGREGATES)
  - `FROM orders |> AGGREGATE SUM(o_totalprice) AS price, COUNT(*) AS cnt GROUP BY EXTRACT(year FROM o_orderdate) AS year`
- Experimental pipe operators which have no standard SQL equivalent
